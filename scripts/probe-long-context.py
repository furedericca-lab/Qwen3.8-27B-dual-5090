...
prompt = (filler * ((args.target_tokens * 4) // len(filler))) + needle
payload = {'model': 'default', 'messages': [{'role': 'user', 'content': prompt + '\nWhat is the verified deployment color? Reply with only the color.'}], 'temperature': 0, 'max_tokens': 32}
...
if usage.get('prompt_tokens', 0) < args.target_tokens * 0.95:
